...
- **单一职责原则(SRP)：** 一个类只有一个引起它变化的原因(职责)，解耦，增强内聚性       
- **开闭原则(OCP)：** 软件实体(包括类、模块、函数等)应该`对于扩展开放，对于修改封闭`        
- **里氏代换原则(LSP)：** 任何父类可以出现的地方，子类一定也可以出现。即是派生类可以替换掉基类。`子类尽量不要重写父类的方法`，解决办法是抽象出一个更基础的类，让父类和子类都去继承那个更基础的类            
- **依赖倒置原则(DIP)：** 开闭原则的基础，要求调用者和被调用者都依赖抽象，两者没有直接的关联和接触，一方变动不会影响另一方的变动。因为抽象的东西要稳定得多。`面向接口编程`         
- **接口隔离原则(ISP)：** 使用多个隔离的接口比使用单接口好，目的就是降低类之间的耦合度，便于软件升级和维护。`接口最小化`     
- **迪米特原则(最少知道原则)：** `一个对象应对其他对象有尽可能少的了解`，使系统的功能模块功能独立，相互之间很少有依赖关系         
- **合成/聚合复用原则(CARP)：** 合成复用原则，继承的耦合性比组合更大，组合只是引用类的方法，可以减少耦合，同时也实现了复用。`使用组合关系而不是继承关系，以减少耦合`     
