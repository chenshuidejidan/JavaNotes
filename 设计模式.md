...
3. **双重检测/双重校验锁(DCL)：** 采用双锁机制，安全且在多线程情况下能保持高性能
   - volatile防止指令重排，确保引用指向内存前实例初始化完毕，而可见性已经由synchronized保证了(https://blog.csdn.net/FU250/article/details/79721197)     
   ...
   - a—>b—>c顺序执行不会有什么问题，但是如果JVM和CPU把指令顺序优化为a—>c—>b，当执行完a,c后，可能另一个线程在第一次判断singleton=null，但此时不为空了，不用进入synchroniezd，于是就**将未初始化完毕的instance对象返回**了，可能触发空指针异常
