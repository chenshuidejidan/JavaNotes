...
**间隙锁**：加在索引之间的锁。。。。间隙锁的一个很大的作用是在`RepeatableRead`级别下，**可以防止幻读**！！！将前后的间隙都锁住，这样新的值就没有办法插入，就不会发生幻读。**一方面防止间隙有新的数据插入，另一方面防止已存在的数据更新成间隙内的数据**

- 在RR级别下，会有间隙锁，如下：

  > 例如数据库： id=2,number=0   id=4,number=2   id=6,number=2 
  > 可以加间隙锁(-∞,2)  (2,4)  (4,6)  (6,+∞)
  > 
  ...
  ~~~
  begin;
  select * from user where id>0 and id <7 for update;
...
6. **临键锁**：临键锁就是 **间隙锁+行锁**，例如`select * from user where id between 2 and 7`，除了加(2,4),(4,6),(6,7)的间隙锁外，还有2和4和6的行锁
