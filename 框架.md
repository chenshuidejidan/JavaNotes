...
![image.png](picture/框架/1606120412129-a4f994c0-4d8d-434b-bbaf-bf181370514e.png)

##　1. 注册中心

比较推荐使用 ZooKeeper 作为注册中心，也可以使用Nacos或者Redis

ZooKeeper提供了高可用、高性能、稳定的分布式数据一致性解决方案。

**ZooKeeper将数据保存在内存中，性能非常好，在读多于写的应用程序中性能尤其的高**，因为写回导致所有服务器间同步状态

**注册中心的功能：** 负责服务地址的注册与查找，服务器启动的时候将服务名称和地址注册到注册中心，服务消费端根据服务名称找到对应的服务地址，有了服务地址之后，服务消费端就可以通过网络请求服务端了
...
远程调用需要发送网络请求来传递目标类和方法的信息、参数等数据到服务提供端

使用基于NIO的网络编程框架 Netty
...
网络传输数据是二进制的，**Java对象没办法直接在网络中传输，所以要序列化为二进制数据**，最后对方收到之后再进行反序列化，将二进制数据转化为Java对象

JDK自带的序列化，只需要实现 `java.io.Serializable` 接口即可，不过这种方式不推荐，因为不支持跨语言调用，并且性能差

比较常见的序列化有：`hessian`, `kyro`, `protostuff`



**序列化的应用场景：** 对象在网络传输时（RPC），将对象存储到文件，将对象存储到数据库
...
代理模式：给某一个对象提供一个代理，有对象的代理来代理真实对象做一些事情。

**RPC 的主要目的就是让我们调用远程方法像调用本地方法一样简单**，我们不需要关心远程方法调用的细节比如网络传输

调用远程方法的时候，实际会通过动态代理对象来传输网络请求



## 5. 负载均衡

负载均衡为了避免单点服务器压力过大造成的宕机、崩溃等问题



## 6. 传输协议

还需要设计一个私有的RPC协议，是客户端和服务端交流的基础
