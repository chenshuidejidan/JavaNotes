...
tcp的keepalive机制用于在双方**长时间未通讯时**，探测**对方是否还活着**。如果对方还活着，就会发送响应，收到响应后就刷新保活定时器。如果没收到响应，就会间隔一定事件后重发，直到收到响应，或者探测次数达到上限则认为对端不可达，将连接关闭。在探测过程中，对端主要处于下面四种状态之一：
...
3. 由于select返回后会清空没有时间的fd所在的位，所以select函数在**每次调用之前都要对参数进行重新设定**，这样做比较麻烦，而且会降低性能
