...
8个字节的`markword`，接下来4个字节是指向对象所属Class类型的`类型指针`，最后4个字节是为了8字节对齐的填充padding。一共占用16个字节的堆内存
...
**并发标记：** 从GC Roots的直接关联对象开始**遍历整个对象图，找到所有存活的对象**（4，5），如果某个对象的引用发生了变化，标记为dirty，需要后续重新标记（3）。并发标记耗时长，但是不需要STW
