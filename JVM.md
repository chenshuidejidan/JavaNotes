...
- **破坏双亲委派机制**：`重写java.lang.ClassLoader.loadClass()方法`可以打破，但是无法破坏核心类库的加载，jdk还提供了额外的保护
  ...
  - 3. 热部署
  - `DrvierManager破坏双亲委派机制`：**Driver接口定义在jdk中**，其实现由不同的数据库服务商提供，**DriverManager(也是由jdk提供)** 要加载各个实现了Driver接口的实现类，然后进行管理，但是**DriverManager由启动类加载器加载**，而**Driver接口的实现类**是由服务商提供，需要由**系统类加载器**加载，**根据类加载机制，当被加载的类引用了另一个类的时候，虚拟机就会使用加载第一个类的加载器区加载被引用的另一个类**，但是启动类加载器没法加载Driver的实现类了，所以这时启动类加载器就要委托子类加载器加载Driver的实现，从而破坏双亲委派机制。
...
**并发标记：** 从GC Roots的直接关联对象开始**遍历整个对象图，找到所有存活的对象**（4，5），如果某个对象的引用发生了变化，标记为dirty，需要后续重新标记（3），耗时长，但是不需要STW
...
**缺点：** **内存碎片**，导致内存空间不规整，无法存放大对象。低延迟导致**吞吐量降低**。**无法处理浮动垃圾（并发标记阶段用户线程产生的新垃圾无法标记清理）**。所以jdk9启用，jdk14直接删除了
...
- G1跟踪各个Region里的垃圾堆积的价值大小(**回收可获得的空间大小以及回收所需时间**的经验值)，维护一个优先级列表，**每次根据允许的收集时间，优先回收价值最大的Region**，保证在有效的时间内获得尽可能高的收集效率
...
`-XX:ParallelGCThread` 设置并行垃圾收集(STW)线程数，最多8
