...
1. **初始标记：** `STW`，**仅仅是标记出GC Roots能直接关联到的对象(存活对象)**，包含**老年代中所有被GCRoots直接关联的对象（1），和被GCRoots关联的存活的新生代对象直接关联到的老年代对象（2，3)**，标记完后就恢复之前被暂停的所有用户线程，时间非常短暂

![初始标记](picture/JVM/初始标记.png)

2. **并发标记：** 从GC Roots的直接关联对象开始**遍历整个对象图，找到所有存活的对象**（4，5），`如果某个老年代对象的引用发生了变化，标记为dirty`，需要后续`重新标记（3）`。并发标记耗时长，但是不需要STW。
   ![并发标记](picture/JVM/并发标记.png)
3. **并发预清理：** 为了让重新标记的时间尽可能短，这时候老年代中被标记为dirty的卡页中的对象会被重新标记，然后清除掉dirty状态。由于这个阶段是并发的，执行过程中依然会有引用关系发生变化，依然可能会有dirty页存在
4. **重新标记：** `STW`，**修正并发标记期间因用户程序运行而导致标记变动的那一部分对象的标记记录**，比初始标记阶段稍长
5. **并发清理：** 清理标记阶段判断的已经死亡的对象，释放内存空间
...
**缺点：** **`内存碎片`**，导致内存空间不规整，无法存放大对象。低延迟导致**吞吐量降低**。**无法处理浮动垃圾（并发标记阶段用户线程产生的新垃圾无法标记清理）**。所以jdk9弃用，jdk14直接删除了
...
**如何确保新生代对象被老年代对象引用的时候，在MinorGC的时候不会清除这个新生代对象？**
...
- **把堆内存分为很多个Region**，Region在物理上可以是不连续的，用不同的Region表示 Eden、Survivor、Old 以及存放大对象的 Humongous（`对象大小超过0.5个Region就放到H`）。各个Region都可以进行并发的回收
...
1. **Young GC：** Eden区用尽时开始，**STW，并行的独占式收集**。**选定`所有年轻代`里的Region**，从年轻代区间移动存活对象到Survivor或Old
2. **Concurrent Marking：** 当堆内存使用率超过阈值(默认45%)，开始老年代并发标记过程。过程和CMS类似：
   - 初始标记：STW，标记从GCRoot开始直接可达的对象
   - 并发标记：从初始标记对象开始标记，标记线程和应用程序并行执行，并且收集各个Region的存活对象信息
   - 重新标记：STW，重新标记那些在并发标记阶段发生变化的对象
   - 垃圾清理(计算排序)：清空Region(**只对没有存活的对象的**)，加入到free list，由于只操作没有存活对象的Region，所以不需要STW

3. **Mixed GC：** 选定所有年轻代Region和部分老年代Region，从老年代移动存活对象到空闲区间，这些空闲区间也就成为了老年代的一部分。**`一次只回收一小部分`老年代的Region**(根据`global concurrent marking`结果的优先级列表来选定，回收数量受gc时间的限制)，同时老年代Region和年轻代是一起回收的

#### Pause Prediction Model

停顿预测模型。G1 GC是一个响应时间优先的GC算法，它与CMS最大的不同是，用户可以设定整个GC过程的期望停顿时间，参数-XX:MaxGCPauseMillis指定一个G1收集过程目标停顿时间，默认值200ms。
