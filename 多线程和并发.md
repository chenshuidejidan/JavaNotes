...
- **是资源分配的单位**      
- 开销：每个进程由独立的代码和数据空间(进程上下文)，进程之间的切换有较大的开销  
...
## 2.5 终止线程
一般来说，线程执行完毕就会结束，无需手动关闭，如果我们要手动关闭一个正在运行的线程，可以调用`Thread.stop()`方法，该方法直接强制立即结束，已经被废弃，不推荐使用了，而应该使用线程中断
...
![偏向锁和轻量级锁的升级过程](https://s1.ax1x.com/2020/11/10/Bq71h9.jpg)
...
当获得锁的线程运行中嗲用object.wait()方法后，就将线程移动到WaitSet中，释放锁，调用notify()后在WaitSet中随机唤醒一个线程进入EntrySet中，以统参与锁竞争
...
## 9.2 实现原理
- ThreadLocalMap是ThreadLocal的⼀个内部类，Thread为每个线程维护了ThreadLocalMap，⽤Entry类来进⾏存储，我们的值都是存储到这个Map上的。**ThreadLocalMap的key是当前线程，value是存储的数据**
~~~java
...
如果ThreadLocalRef消失了，ThreadLocal对象就会被gc，之后就会存在一个key为null的value，无法访问
如果这个Map积攒越来越多，就会造成内存泄漏
所以ThreadLocal里面的对象不用了之后，无比要remove掉
~~~java
