...
**偏向锁解锁不会修改还对象头的thread id**，**一个对象只有一次机会获得偏向锁**
...
- AQS是一个**可以实现锁的框架**，内部实现依靠**队列**和**state状态**
  ...
  - 若线程2来，CAS的时候发现state已经为1，判断一下加锁的线程不是自己（被线程1加了锁），则没办法加锁了，进入等待队列
...
happens-before表示的是前一个操作的结果对于后续操作是可见的，它表达了多个线程之间对于内存的可见性
