...
镜像是一个只读层，一个镜像是不会发生改变的。所有的变更都发生在顶层的可写层，下层的原始只读镜像不会改变
...
- **Proposer**: 提出提案 (Proposal)。Proposal信息包括`全局唯一递增的提案编号 (Proposal ID)` 和`提议的值 (Value)`
...
1. 第一阶段：Prepare阶段。Proposer向Acceptors发出Prepare请求，Acceptors针对收到的Prepare请求进行Promise承诺(**Acceptor承诺不再接受小于等于当前Proposal ID的Prepare请求，不再接受小于当前Proposal ID的Propose请求**，所以之后会应答Accept过的Proposal ID最大的提案)。
2. 第二阶段：Accept阶段。Proposer收到多数Acceptors承诺的Promise后，向Acceptors发出Propose请求，Acceptors针对收到的Propose请求进行Accept处理(判断该Propose是不是Accept过的Proposal ID最大的提案，是则接受，否则不接受该Proposal)。
...
- Leader，负责Client交互和log复制，同一时刻系统中最多存在1个Leader
- Follower，只被动响应其他服务器的请求，从不主动发起请求
- Candidate，由Follower向Leader转换的中间状态。**正常工作期间只有Leader和Follower节点**

![raft算法角色状态转换图](picture/goWeb与微服务/raft算法角色状态转换图.jpg)

一个最小的Raft集群需要三个参与者，这样才可以投出多数票

**所有的节点最初都是follower**。在这种状态下，节点可以接受来自leader的日志消息并进行投票

如果follower在一段时间内没有收到leader的消息，节点将自动提升到`Candidate`状态，并开始一次Leader的选举，收到大多数服务器投票的Candidate将会成为新的Leader。
...
1. Raft 使用`心跳（heartbeat）`触发Leader选举。Leader向所有Followers周期性发送heartbeat。如果Follower在选举超时时间内没有收到Leader的heartbeat，就会等待一段随机的时间后发起一次Leader选举
...
- 拥有最新的已提交的log entry的Follower才有资格成为Leader（**如果投票时发现自己的日志比请求中携带的更新，则拒绝投票**）
