...
func mapaccess1(t *maptype, h *hmap, key unsafe.Pointer) unsafe.Pointer {
	...
	if c := h.oldbuckets; c != nil {
        ...
        // tophash[0]如果为2， 3， 4，表示这个bmap已经搬走了，正常的tophash>=5，或者0，1表示为空
		if !evacuatd(oldb) {
...
1. 没有在搬迁
2. 装载因子已经超过 6.5；
3. 哈希使用了太多溢出桶 `noverflow >= uint16(1)<<(min(B, 15)&15)`；
...
删除和写入逻辑类似，如果删除期间遇到了哈希表迁移，就会分流桶中元素，分流结束之后会找到桶中的目标元素完成键值对的删除操作
...
- **命名返回值：就好比函数参数一样，函数体内对命名返回值的任何修改，都会影响它。**
- **非命名返回值：取决于 return 时候的值**，之后defer的修改不能影响到返回值了
...
![](picture/go语言要点/2021-08-11_152842.png)
...
1. 有发送阻塞g，`读取g`或者`缓冲区的内容`，然后给当前 Goroutine 设置成下一个运行的 Goroutine；
2. 如果 Channel 的缓冲区中包含数据，那么直接读取 `recvx` 索引对应的数据；
3. 在默认情况下会挂起当前的 Goroutine，将 [`runtime.sudog`](https://draveness.me/golang/tree/runtime.sudog) 结构加入 `recvq` 队列并陷入休眠等待调度器的唤醒；
...
[(5条消息) 【我的架构师之路】- golang源码分析之select的底层实现_GavinXujiacan的博客-CSDN博客](https://blog.csdn.net/qq_25870633/article/details/83339538)

select的基本结构：**如果有default则不会阻塞，如过没有default则是阻塞调用，阻塞直到一个通道有事件为止**（关闭的chan的读操作是直接就绪的）
...
使用sync.Once确保只关闭一次（多次关闭会直接panic）



1. 如果是1个sender：直接sender端关闭
2. 如果多个sender，1个receiver：增加一个传递关闭信号的channel，receiver通过信号channel下达关闭channel的指令，senders监听到关闭信号后停止发送数据。下面代码中sender也没有明确关闭，因为channel会被gc回收的。或者可以使用`context`，也可以达到同样的效果
...
<img src="picture/go语言要点/channel结构.png" alt="channel结构" style="zoom:50%;" />
...
<img src="picture/go语言要点/channel接受和发送数据.png" alt="channel接受和发送数据" style="zoom:80%;" />
...
close 逻辑比较简单，对于一个 channel，recvq 和 sendq 中分别保存了阻塞的发送者和接收者。关闭 channel 后，会**设置关闭的标志位**，**唤醒那些等待着的goroutine**，对于**等待接收者**而言，会收到一个相应类型的**零值**。对于**等待发送者，会直接 panic**。所以，在不了解 channel 还有没有接收者的情况下，不能贸然关闭 channel。

```go
func chansend(c *hchan, ep unsafe.Pointer, block bool, callerpc uintptr) bool {
        //  ...
        // 当 channel close 之后， send 会直接 panic
        if c.closed != 0 {
            unlock(&c.lock)
...
4. 互斥锁在正常情况下会通过 [`runtime.sync_runtime_SemacquireMutex`](https://draveness.me/golang/tree/runtime.sync_runtime_SemacquireMutex) 将尝试获取锁的 Goroutine 切换至休眠状态，等待锁的持有者唤醒；
5. 如果当前 Goroutine 是互斥锁上的最后一个等待的协程或者等待的时间小于 1ms，那么它会将互斥锁切换回正常模式；

unlock：

1. 当互斥锁已经被解锁时，调用 [`sync.Mutex.Unlock`](https://draveness.me/golang/tree/sync.Mutex.Unlock) 会直接抛出异常；
2. 当互斥锁处于饥饿模式时，将锁的所有权交给队列中的下一个等待者，等待者会负责设置 `mutexLocked` 标志位；
3. 当互斥锁处于普通模式时，如果没有 Goroutine 等待锁的释放或者已经有被唤醒的 Goroutine 获得了锁，会直接返回；在其他情况下会通过 [`sync.runtime_Semrelease`](https://draveness.me/golang/tree/sync.runtime_Semrelease) 唤醒对应的 Goroutine；



![](picture/go语言要点/2021-08-11_210105.png)
...
type RWMutex struct {
	...
	readerSem   uint32
	readerCount int32
	readerWait  int32    //当前写操作阻塞时前面的读操作数
}
```
...
<img src="picture/go语言要点/强三色不变式.png" alt="强三色不变式" style="zoom: 50%;" />

**弱三色不变式：** 黑色对象可以指向白色对象，但此白色对象链路的上游中必须有一个灰色对象(白色对象必须被灰色对象"保护")

<img src="picture/go语言要点/弱三色不变式.png" alt="弱三色不变式" style="zoom: 25%;" />
...
>  Dijkstra 必须**为栈上的对象增加写屏障**或者**在标记阶段完成重新对栈上的对象进行扫描**，这两种方法各有各的缺点，前者会大幅度增加写入指针的额外开销，后者重新扫描栈对象时需要暂停程序，垃圾收集算法的设计者需要在这两者之间做出权衡。
...
- GC mark期间，任何在**栈上或堆上**创建的新对象，均为**黑色**
- 对堆对象进行混合写屏障
- 对栈对象不适用混合写屏障
...
注意参数 `gcWriteBarrier` 传值在 golang 里是个特例，这里用的是寄存器（ golang 的惯例是用栈来传递的，但是考虑性能原因，这里必须用寄存器了），

`runtime·gcWriteBarrier` 函数干啥的：

1. 先把slot和ptr入队 `wbBuffer`
2. 如果 `wbBuffer` 满了就调用 `runtime.wbBufFlush` 进行批量置灰，然后赋值；否则就只是入队，然后赋值
3. 然后赋值 `*slot = ptr`

那为什么上面 `gcWriteBarrier` 这个函数怎么复杂？

其实是做的一个优化处理，每次触发写屏障的时候（ hook ），我们当然可以直接shade（ptr），但是我们知道，毕竟这段写屏障的代码是比业务多出来的，这些都是开销，我们能快就快，每次这样做太零散，我们可以攒一批，一批队列满了，一批去入队，置灰色。这样效率更高。`一般情况下，只需要简单入队就行了，buf 满了之后，才 flush 去批量置灰，这样写屏障对业务的影响就更小了，wbBuffer 就是这个队列的实现。`
...
- `申请内存`：当进行 `mallocgc`时如果用到了mcentral 之后的缓存，就尝试调用 `gcTrigger{kind: gcTriggerHeap}.test()` 进行检查。当前分配的内存达到一定**阈值**（上一次gc时内存的2倍）时触发，这个阈值在每次GC过后都会根据堆内存的增长情况和CPU占用率来调整（目前唯一**可以调整的参数GCPercent, 以后可能会有MaxHeap**）runtime.mallocgc，微对象、小对象、大对象的创建都可能会触发新的垃圾收集循环
    - `nextFree()`从mcentral或者mheap获取新的mspan时可能会触发垃圾收集
    - 分配大对象的时候，一定会尝试触发垃圾收集
- `runtime.forcegcperiod`: 由  `sysmon` 循环检查，当一定时间没有执行过GC就强制触发GC（2分钟）
- `runtime.GC()`: 手动触发

![](picture/go语言要点/2021-08-11_190443.png)
...
- `schedule()`：找到可用的g，然后调用 `execute()`

- `execute()`：设置该g的初始状态，然后调用`gogo`
- `gogo`：用汇编实现，先把 `goexit` 的PC值压栈，然后执行当前g，等g的函数返回时，就获取到 `goexit` 的起始地址弹出到PC中，开始执行 `goexit`
- `goexit()`：该函数会将 Goroutine 转换会 `_Gdead` 状态、清理其中的字段、移除 Goroutine 和线程的关联并调用 [runtime.gfput](https://draveness.me/golang/tree/runtime.gfput) 重新加入处理器的 Goroutine 空闲列表 `gFree`。最后调用 `schedule` 进行新一轮的调度
...
假设有下面的Go代码，程序执行之初将P设置为1，开两个goroutine，一个是main，一个是执行死循环的匿名函数
...
在go1.12前，goroutine是协作式调度，所以main在sleep的时候让出执行权给匿名goroutine执行，匿名goroutine死循环出不去，又无法被抢占，就会使得程序陷入无限循环，不会结束



Go调度在go1.14实现了抢占，应该更精确的称为**基于系统信号的异步抢占调度**，因为go调度器的抢占很柔和，不会说时间片到了或者更高优先级的任务到了就直接执行抢占调度，**go的抢占柔和到只给goroutine发送1个抢占请求，至于goroutine何时停下来，那就不管了**
...
调度器在启动的时候会启动一个单独的线程`sysmon`，它负责所有的监控工作，其中一项就是 `抢占`，发现满足抢占条件的G时，就会向当前线程发出抢占信号
...
[标准库 - 《Go 语言问题集(Go Questions)》 - 书栈网 · BookStack](https://www.bookstack.cn/read/qcrao-Go-Questions/stdlib.md)
